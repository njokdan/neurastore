...
fn main() {
    ...
    println!("Inserting (batched -- one WAL fsync for the whole load, see Wal::append_batch docs)...");
    let insert_start = Instant::now();
    let entries: Vec<(u64, Vec<f32>, HashMap<String, String>)> = base
        .iter()
        .enumerate()
        .map(|(i, vector)| {
    ...
    let throughput = base.len() as f64 / insert_elapsed;
    ...
    println!("Running unfiltered queries...");
    ...
    print_latency_summary("NeuraStore unfiltered query latency", latencies);
    let avg_recall: f64 = recalls.iter().sum::<f64>() / recalls.len() as f64;
    println!("NeuraStore unfiltered recall@{k}: {avg_recall:.3}");
